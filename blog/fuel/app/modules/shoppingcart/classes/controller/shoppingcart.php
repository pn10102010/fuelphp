...
class Controller_ShoppingCart extends \Controller
{
    ...
    protected static $total_qty = 0;
    protected static $total_price = 0;
     ...
     * @return bool
     */
    public static function action_add($items = array())
    {
    ...
    private static function _add($item = array())
    {
        ...
        if (!isset($item['qty']) OR !isset($item['price']) OR !isset($item['id']))
        {
            return FALSE;
        }
        ...
        if (!preg_match("/^[" . static::$product_id_rules . "]+$/i", $item['id']))
        {
            return FALSE;
        }
